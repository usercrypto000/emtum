...
The assignment boundary is covered by a bounded stateful fuzz model in `TaskIntentMarketStatefulFuzz.t.sol`. The model runs arbitrary task opening, claiming, and cancellation sequences across fuzzed actors, then asserts that task IDs remain monotonic, assigned and cancelled tasks remain terminal, and onchain task state matches the model after each generated sequence.
